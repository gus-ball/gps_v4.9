...
def submitMapDSettings(response):
    print(response.get_field("timelim"))
    time = str(float(response.get_field("timelim"))*3600)
    print(time)
    if time != float([line for line in open("options.txt")][1]):
...
def getUUIDList(response):
    ...
    ids=[int(i[0]) for i in cur.fetchall()]
    data=[]
    for id in ids:
    	cur.execute("SELECT DISTINCT Name FROM Assignments WHERE UUID=?",(id,))
    	data.append([id,str(cur.fetchall()[0][0])])
